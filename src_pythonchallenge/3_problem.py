...
PBuijeoTSpsVLaOGuLVjMZXkBvVXwUuHfBihziiavGSYofPNeKsTXruMUumRRPQJzvSzJkKbtSipiqBd""".split('\n')

# looking for 3 uppercase characters to the left,right,up,down of a lowercase character
for y in range(3, len(text)):
    for x in range(3, len(text[y]) - 3):
        if text[y][x].islower() and text[y][x-3:x].isupper() and text[y][x+1:x+4].isupper():
            assert len(text[y][x+1:x+4]) == 3 and len(text[y][x-3:x])
            if text[y-1][x].isupper() and text[y-2][x].isupper() and text[y-3][x].isupper():
                if text[y+1][x].isupper() and text[y+2][x].isupper() and text[y+3][x].isupper():
                    print(text[y][x-3:x+4])